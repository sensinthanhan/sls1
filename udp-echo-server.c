...
static 	void send_reply_hello (cmd_struct_t res, uint8_t encryption_en);
...
static void make_packet_for_node(cmd_struct_t *cmd, uint8_t* key, uint8_t encryption_en) {
	...
	if (encryption_en==TRUE) {
    	...
    	PRINTF("Data = [");
    	for (i=0; i<=MAX_CMD_LEN; i++) {PRINTF("%02X ", *((uint8_t *)cmd+i));}
    	PRINTF("]\n");
...
/*---------------------------------------------------------------------------*/
static void make_packet_for_node_hello(cmd_struct_t *cmd, uint8_t* key) {
	uint8_t i;
    PRINTF("Key = [");
    for (i=0; i<=15; i++) {	PRINTF("%02X,", *(key+i)); 	}
    PRINTF("]\n");
...
/*---------------------------------------------------------------------------*/
static void check_packet_for_node_hello(cmd_struct_t *cmd, uint8_t* key) {
	// uint8_t i;
 //    PRINTF("Key = [");
 //    for (i=0; i<=15; i++) {	PRINTF("%02X,", *(key+i)); 	}
 //    PRINTF("]\n");
...
static void tcpip_handler(void)	{
	...
	memset(buf, 0, MAX_PAYLOAD_LEN);
  	if(uip_newdata()) {
		...
		if (state == STATE_HELLO){
			check_packet_for_node_hello(&cmd, key_begin);
		}
		// data decryption
		...
		if (is_cmd_of_nw(cmd)) {
			...
			/* get a HELLO, do not check sequence */
				...
				send_reply_hello(reply, encryption_phase);
			} 
...
/*---------------------------------------------------------------------------*/
static void tcpip_handler_hello(void)	{
	//char *search = " ";
	memset(buf, 0, MAX_PAYLOAD_LEN);
  	if(uip_newdata()) {
  		//blink_led(BLUE);
...
static void send_reply_hello(cmd_struct_t res, uint8_t encryption_en) {
	...
	make_packet_for_node_hello(&response, key_begin);
...
// 		//for (i=0; i<sizeof(env_db); i++)	printf("%d, ",emer_reply.arg[i]);
// 		//printf("\n");
// 	}
// #endif
...
static void et_timeout_hanler(){
	...
	/* send an async msg every SEND_ASYN_MSG_PERIOD/10 seconds */
	if ((timer_cnt % (SEND_ASYN_MSG_PERIOD / 10) )==0) {		
		...
		if ((state==STATE_NORMAL) && (emergency_status==TRUE) && (net_db.authenticated==TRUE)) {	
			...
			// process_sensor(FALSE);		//read sensor if having shield
...
static uint8_t is_connected() {
    ...
    if(dag && dag->instance->def_route)	return TRUE;
    else								return FALSE;
}
...
static void get_next_hop_addr(){
    ...
    if(dag && dag->instance->def_route) {
	    ...
	    //for (int i=0; i<sizeof(uip_ipaddr_t);i++) {PRINTF("0x%02X ", net_db.next_hop[i]);}
	    //PRINTF("\n");
    } 
#endif        
}
...
// 			if(light < 5)	{set_led_cc2538_shield(TRUE);} 
// 			else 			{set_led_cc2538_shield(FALSE);}
// 			*/

//     	} else {PRINTF("Error, enable the DEBUG flag in the tsl256x driver for info, or check if the sensor is properly connected \n");}		
...
//     	} else {PRINTF("Error, enable the DEBUG flag in the BMPx8x driver for info, or check if the sensor is properly connected \n");}	
